...
def quantum_brute_force(public_key_x: int, g_x: int, g_y: int, p: int, min_range: int, max_range: int):
    ...
    while True:
        ...
        try:
            # BUAT OPTIONS DENGAN STRUKTUR YANG BENAR (UNTUK API BARU)
            options = Options()
            # CARA YANG BENAR UNTUK MENGATUR SHOTS PADA VERSI TERBARU
            # JANGAN gunakan: options.execution.shots (sudah tidak valid)
            # JANGAN gunakan: options.optimization_level (sudah tidak valid di sini)
            
            # Coba pendekatan pertama: atur shots langsung di options dengan cara yang benar
            try:
                # Untuk versi Qiskit yang lebih baru, gunakan default_shots
                options.default_shots = 1024
                print(f"Set shots using 'default_shots' = {options.default_shots}")
            except AttributeError:
                # Jika tidak berhasil, coba pendekatan lain
                print("'default_shots' not available, trying alternative...")
                # Tidak atur shots di options sama sekali, akan atur di run()
            ...
            # Jalankan circuit dengan dua cara berbeda
            try:
                # Cara 1: Dengan shots yang sudah diatur di options
                print(f"Running circuit with shots from options...")
                pub = (transpiled_circuit,)
                job = sampler.run([pub])
            except Exception as run_error:
                # Cara 2: Atur shots langsung di parameter run
                print(f"First run method failed: {run_error}")
                print("Trying alternative: setting shots directly in run parameters...")
                pub = (transpiled_circuit, 1024)  # Shots sebagai parameter kedua
                job = sampler.run([pub])
            ...
            print("Search stopped due to persistent configuration error.")
            print("Please check the SamplerV2 API configuration in your code.")
            return None
