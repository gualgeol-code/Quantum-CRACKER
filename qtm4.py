...
def quantum_brute_force(public_key_x: int, g_x: int, g_y: int, p: int, min_range: int, max_range: int):
    ...
    while True:
        ...
        # =========== PERBAIKAN UTAMA: Menggunakan Session dan Options ===========
        print("\nCreating Session and SamplerV2...")
        
        try:
            # OPTION 1: Menggunakan Session (lebih direkomendasikan)
            with Session(backend=backend) as session:
                print("Session created successfully")
                
                # Buat Options untuk konfigurasi
                options = Options()
                options.execution.shots = 1024  # Kurangi shots untuk testing
                options.optimization_level = 1
                
                # Buat SamplerV2 dalam session
                ...
                # Tidak perlu parameter_values jika circuit tidak punya parameter
