...
def quantum_brute_force(public_key_x: int, g_x: int, g_y: int, p: int, min_range: int, max_range: int):
    ...
    while True:
        ...
        # =========== PERBAIKAN UTAMA: Menggunakan SamplerV2 sesuai API terbaru ===========
        print("\nMenyiapkan SamplerV2 sesuai API terbaru...")

        try:
            # 1. BUAT SAMPLER TANPA OPTIONS (atau dengan Options kosong)
            # Untuk API terbaru, shots diatur saat memanggil .run(), BUKAN di Options.
            ...
            # Ekstrak counts dari hasil - format tergantung backend
            # Untuk IBM hardware, biasanya data ada di .data.c
            counts = {}
            if hasattr(result[0].data, 'c') and result[0].data.c is not None:
                ...
                for i, count_val in enumerate(counts_array):
                    if count_val > 0:
                        ...
                        counts[bin_key] = int(count_val)  # Konversi ke int
