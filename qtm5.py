...
def quantum_brute_force(public_key_x: int, g_x: int, g_y: int, p: int, min_range: int, max_range: int):
    ...
    try:
        ...
        if available_hardware:
            ...
            # Gunakan nama simulator yang benar
    ...
    while True:
        ...
        try:
            # BUAT OPTIONS TANPA SESSION
            options = Options()
            options.execution.shots = 1024  # Kurangi shots untuk testing
            ...
            # Pastikan fallback ke simulator menggunakan nama yang benar
            print("Trying with simulator instead...")
            try:
                # NAMA SIMULATOR YANG BENAR ADALAH 'ibmq_qasm_simulator'
                backend = service.backend("ibmq_qasm_simulator")
                print(f"Switched to simulator: {backend.name}")
                # Continue ke loop berikutnya untuk coba lagi dengan simulator
                continue
            except Exception as sim_error:
                print(f"Failed to switch to simulator: {sim_error}")
                return None
