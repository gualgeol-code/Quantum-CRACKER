...
def quantum_brute_force(public_key_x: int, g_x: int, g_y: int, p: int, min_range: int, max_range: int):
    ...
    while True:
        ...
        try:
            # BUAT OPTIONS DENGAN STRUKTUR YANG BENAR
            options = Options()
            # CARA YANG BENAR UNTUK MENGATUR SHOTS PADA OPTIONSV2
            options.execution.shots = 1024  # <-- PERUBAHAN INI
            ...
            # Jalankan circuit
            print(f"Running circuit with {options.execution.shots} shots...")
            ...
            # Coba alternatif jika ada error dengan options.execution.shots
            try:
                print("Trying alternative approach without setting shots in options...")
                
                # Coba tanpa mengatur shots di options
                options = Options()
                options.optimization_level = 1
                
                sampler = SamplerV2(backend=backend, options=options)
                print("SamplerV2 created successfully (without shots setting)")
                
                # Atur shots langsung saat menjalankan circuit
                print("Running circuit with 1024 shots (set at run time)...")
                pub = (transpiled_circuit, 1024)  # <-- Shots ditentukan di sini
                job = sampler.run([pub])
                ...
                # Ekstrak counts dari hasil (sama seperti sebelumnya)
                counts = {}
                if hasattr(result[0].data, 'c') and result[0].data.c is not None:
                    counts_array = result[0].data.c
