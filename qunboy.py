#i realy hope you get me some Donation for the Quantum project_ 1NEJcwfcEm7Aax8oJNjRUnY3hEavCjNrai /////
import cupy as cp  # CUDA-accelerated operations using CuPy
from qiskit import QuantumCircuit, transpile
from qiskit.circuit.controlflow.break_loop import BreakLoopPlaceholder
from qiskit.circuit.library import ZGate, MCXGate
from qiskit_ibm_runtime import QiskitRuntimeService, SamplerV2, Session, Options
from qiskit.primitives import SamplerResult
from qiskit.primitives.containers.primitive_result import PrimitiveResult
from collections import Counter
from Crypto.Hash import RIPEMD160, SHA256  # Import from pycryptodome
from ecdsa import SigningKey, SECP256k1
from qiskit.quantum_info import Statevector
from bitarray import bitarray
import random
import time
import hashlib
import base58
import numpy as np
from qiskit.visualization import plot_histogram
import matplotlib.pyplot as plt
from qiskit import QuantumRegister, ClassicalRegister, AncillaRegister
from qiskit.circuit.library import QFT
from qiskit.quantum_info import Operator
import math

# Load IBMQ account using QiskitRuntimeService dengan instance yang spesifik
QiskitRuntimeService.save_account(
    channel='ibm_quantum_platform',
    token='45dVrviihk2W_qDTnSHrNQfXlp2uM5-E8Fw5X0md9PsK',
    overwrite=True,
    set_as_default=True,
    instance="crn:v1:bluemix:public:quantum-computing:us-east:a/7d8b6f65e3bb4d76ad7af2f598cc70ca:a4a8e2da-5225-42c3-bbaf-34dba6dd020e::"
)

# Load the service dengan instance yang spesifik
service = QiskitRuntimeService(
    instance="crn:v1:bluemix:public:quantum-computing:us-east:a/7d8b6f65e3bb4d76ad7af2f598cc70ca:a4a8e2da-5225-42c3-bbaf-34dba6dd020e::"
)

SECP256K1_ORDER = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141

def apply_qft(circuit, num_qubits):
    qft = QFT(num_qubits)
    circuit.append(qft, range(num_qubits))
    
    for j in range(num_qubits):
        circuit.h(j)
        for k in range(j + 1, num_qubits):
            circuit.cp(np.pi / (2 ** (k - j)), j, k)
    
    for j in range(num_qubits // 2):
        circuit.swap(j, num_qubits - j - 1)

def private_key_to_compressed_address(private_key_hex):
    print(f"Converting private key {private_key_hex} to Bitcoin address...")
    private_key_bytes = bytes.fromhex(private_key_hex)
    sk = SigningKey.from_string(private_key_bytes, curve=SECP256k1)
    vk = sk.verifying_key
    public_key_bytes = vk.to_string()
    x_coord = public_key_bytes[:32]
    y_coord = public_key_bytes[32:]
    prefix = b'\x02' if int.from_bytes(y_coord, 'big') % 2 == 0 else b'\x03'
    compressed_public_key = prefix + x_coord

    sha256_pk = hashlib.sha256(compressed_public_key).digest()
    ripemd160 = RIPEMD160.new()
    ripemd160.update(sha256_pk)
    hashed_public_key = ripemd160.digest()

    network_byte = b'\x00' + hashed_public_key
    sha256_first = hashlib.sha256(network_byte).digest()
    sha256_second = hashlib.sha256(sha256_first).digest()
    checksum = sha256_second[:4]

    binary_address = network_byte + checksum
    bitcoin_address = base58.b58encode(binary_address).decode('utf-8')
    print(f"Generated Bitcoin address: {bitcoin_address}")
    return bitcoin_address

def grover_oracle(circuit, private_key_qubits, public_key_x, g_x, g_y, p, ancilla):
    circuit.barrier()
    num_qubits = 125
    target_state = random.randint(0x4000000000000000000000000000000000, 0x7fffffffffffffffffffffffffffffffff)
    computed_x, _ = scalar_multiplication(target_state, g_x, g_y, p)

    if computed_x == public_key_x:
        circuit.x(ancilla)

    circuit.barrier()

def mod_inverse(a, p):
    if a == 0:
        return 0
    lm, hm = 1, 0
    low, high = a % p, p
    while low > 1:
        ratio = high // low
        nm, new_low = hm - lm * ratio, high - low
        lm, low, hm, high = nm, new_low, lm, low
    return lm % p

def point_addition(x1, y1, x2, y2, p):
    if x1 == x2 and y1 == y2:
        return point_doubling(x1, y1, p)
    lam = ((y2 - y1) * mod_inverse(x2 - x1, p)) % p
    x3 = (lam * lam - x1 - x2) % p
    y3 = (lam * (x1 - x3) - y1) % p
    return x3, y3

def point_doubling(x1, y1, p):
    lam = ((3 * x1 * x1) * mod_inverse(2 * y1, p)) % p
    x3 = (lam * lam - 2 * x1) % p
    y3 = (lam * (x1 - x3) - y1) % p
    return x3, y3

def scalar_multiplication(k, x, y, p):
    x_res, y_res = x, y
    k_bin = bin(k)[2:]
    for bit in k_bin[1:]:
        x_res, y_res = point_doubling(x_res, y_res, p)
        if bit == '1':
            x_res, y_res = point_addition(x_res, y_res, x, y, p)
    return x_res, y_res

def binary_to_hex(bin_key):
    bin_key = bin_key.zfill(128)
    return hex(int(bin_key, 2))[2:].zfill(64)

def quantum_brute_force(public_key_x: int, g_x: int, g_y: int, p: int, min_range: int, max_range: int):
    if max_range <= min_range:
        raise ValueError("max_range must be greater than min_range.")

    target_address = '16RGFo6hjq9ym6Pj7N5H7L1NR1rVPJyw2v'
    quantum_registers = 125
    num_ancillas = 10
    num_iterations = 5  # Kurangi untuk testing, naikkan nanti

    print("Mencari backend yang tersedia...")
    try:
        backends = service.backends()
        print(f"Backends yang tersedia: {[b.name for b in backends]}")
        
        # ==================== PERUBAHAN UTAMA: PILIH ibm_fez SECARA SPESIFIK ====================
        backend_choice = 'ibm_fez'  # Anda bisa ganti dengan 'ibm_marrakesh' jika ingin
        
        try:
            # Coba akses ibm_fez secara spesifik
            backend = service.backend(backend_choice)
            print(f"\n✓ Menggunakan {backend_choice} secara spesifik")
            print(f"  Nama: {backend.name}")
            print(f"  Qubit tersedia: {backend.configuration().n_qubits}")
            print(f"  Status: {backend.status().status_msg}")
            print(f"  Pending jobs: {backend.status().pending_jobs}")
            
            # Tampilkan informasi detail tentang backend
            config = backend.configuration()
            print(f"  Processor: {getattr(config, 'processor_type', 'N/A')}")
            print(f"  Basis gates: {config.basis_gates[:5]}...")  # Tampilkan 5 pertama saja
            
            # Validasi apakah qubit cukup
            required_qubits = quantum_registers + num_ancillas
            if config.n_qubits < required_qubits:
                print(f"\n⚠ PERINGATAN: Backend hanya memiliki {config.n_qubits} qubit")
                print(f"  Circuit membutuhkan {required_qubits} qubit")
                print("  Transpilasi mungkin gagal atau memerlukan optimisasi")
                
                # Tanya apakah ingin melanjutkan
                print("\nApakah ingin melanjutkan? (y/n)")
                # Untuk otomatis, kita lanjut saja tapi dengan catatan
                print("Melanjutkan dengan optimisasi...")
                
        except Exception as specific_error:
            print(f"\n✗ Tidak dapat mengakses {backend_choice}: {specific_error}")
            
            # Fallback: cari backend alternatif
            print("Mencari backend alternatif...")
            hardware_backends = [b for b in backends if not b.configuration().simulator]
            
            if hardware_backends:
                # Pilih backend dengan qubit terbanyak sebagai fallback
                backend = max(hardware_backends, key=lambda b: b.configuration().n_qubits)
                print(f"✓ Fallback ke backend dengan qubit terbanyak: {backend.name}")
                print(f"  Qubit: {backend.configuration().n_qubits}")
            else:
                raise Exception(f"Tidak dapat mengakses {backend_choice} dan tidak ada backend alternatif")
            
    except Exception as e:
        print(f"\n✗ Error mencari backend: {e}")
        print("Mencoba menggunakan simulator...")
        
        # Cari simulator sebagai last resort
        simulators = [b for b in backends if b.configuration().simulator]
        if simulators:
            backend = simulators[0]
            print(f"✓ Menggunakan simulator: {backend.name}")
        else:
            raise Exception("Tidak ada backend yang tersedia")

    attempt = 0
    
    while True:
        attempt += 1
        print(f"\n{'='*60}")
        print(f"Attempt {attempt} - Using backend: {backend.name}")
        print(f"Qubit available: {backend.configuration().n_qubits}")
        print(f"Pending jobs: {backend.status().pending_jobs}")
        print(f"{'='*60}")

        circuit = QuantumCircuit(quantum_registers + num_ancillas, quantum_registers)
        ancilla_register = QuantumRegister(num_ancillas, name='ancilla')
        circuit.add_register(ancilla_register)
        print("Quantum circuit initialized.")

        circuit.h(range(quantum_registers))
        print("Hadamard gates applied.")

        print(f"Applying Grover's iterations ({num_iterations} iterations).")
        for _ in range(num_iterations):
            grover_oracle(circuit, range(quantum_registers), public_key_x, g_x, g_y, p, ancilla_register[0])

        circuit.measure(range(quantum_registers), range(quantum_registers))
        print("Measurement operation added to circuit.")

        print("Transpiling the circuit for the selected backend...")
        try:
            transpiled_circuit = transpile(circuit, backend=backend, optimization_level=3)
            print(f"✓ Circuit transpiled successfully")
            print(f"  Depth: {transpiled_circuit.depth()}")
            print(f"  Width: {transpiled_circuit.width()}")
            print(f"  Gates: {transpiled_circuit.count_ops()}")
        except Exception as transpile_error:
            print(f"✗ Transpilation failed: {transpile_error}")
            
            # Coba dengan optimization level lebih rendah
            print("Trying with lower optimization level...")
            try:
                transpiled_circuit = transpile(circuit, backend=backend, optimization_level=1)
                print(f"✓ Circuit transpiled with optimization_level=1")
                print(f"  Depth: {transpiled_circuit.depth()}")
            except:
                print("✗ Transpilation failed completely")
                print("Waiting 10 seconds before retrying...")
                time.sleep(10)
                continue

        # =========== MENGGUNAKAN SamplerV2 ===========
        print("\nMenyiapkan SamplerV2...")
        
        try:
            # Gunakan Session untuk koneksi yang lebih baik
            with Session(service=service, backend=backend) as session:
                sampler = SamplerV2(session=session)
                print(f"✓ SamplerV2 created successfully with Session")
                
                # Jalankan circuit dengan shots
                shots = 10000  # Kurangi untuk testing, bisa dinaikkan nanti
                pub = (transpiled_circuit, None, shots)
                
                print(f"Running circuit with {shots} shots...")
                job = sampler.run([pub])
                
                job_id = job.job_id()
                print(f"✓ Job submitted successfully. Job ID: {job_id}")
                print(f"Job status: {job.status()}")
                
                # Monitor job progress
                start_time = time.time()
                last_status = ""
                
                while not job.in_final_state():
                    current_status = job.status()
                    if current_status != last_status:
                        print(f"Job status: {current_status}")
                        last_status = current_status
                    time.sleep(5)
                    
                    # Timeout setelah 300 detik (5 menit)
                    if time.time() - start_time > 300:
                        print("⚠ Job timeout setelah 5 menit")
                        job.cancel()
                        break
                
                if job.status() == "CANCELLED":
                    print("Job dibatalkan, melanjutkan ke attempt berikutnya...")
                    continue
                
                print("✓ Job completed successfully")
                print(f"Total waktu eksekusi: {time.time() - start_time:.2f} detik")
                
                # Dapatkan hasil
                result = job.result()
                
                # Proses hasil
                counts = {}
                try:
                    # Ekstrak counts dari hasil
                    if hasattr(result[0].data, 'c') and result[0].data.c is not None:
                        counts_data = result[0].data.c
                        
                        # Coba berbagai metode untuk mendapatkan counts
                        if hasattr(counts_data, 'get_counts'):
                            counts_dict = counts_data.get_counts()
                            for bitstring, count in counts_dict.items():
                                if len(bitstring) < quantum_registers:
                                    bitstring = bitstring.zfill(quantum_registers)
                                counts[bitstring] = count
                            print(f"✓ Got {len(counts)} unique measurement results")
                        
                        elif hasattr(counts_data, '__array__'):
                            # Proses sebagai array
                            arr = np.array(counts_data)
                            non_zero_indices = np.where(arr > 0)[0]
                            for idx in non_zero_indices:
                                bin_key = format(idx, f'0{quantum_registers}b')
                                counts[bin_key] = int(arr[idx])
                            print(f"✓ Got {len(counts)} unique measurement results (array method)")
                        
                        else:
                            print("⚠ Unknown data format, trying basic iteration...")
                            try:
                                for i in range(len(counts_data)):
                                    count_val = counts_data[i]
                                    if count_val > 0:
                                        bin_key = format(i, f'0{quantum_registers}b')
                                        counts[bin_key] = int(count_val)
                                print(f"✓ Got {len(counts)} unique measurement results")
                            except:
                                print("✗ Could not extract counts")
                    
                    else:
                        print("⚠ No counts data found")
                        
                except Exception as data_error:
                    print(f"✗ Error extracting counts: {data_error}")
                    import traceback
                    traceback.print_exc()
                
                # Cek setiap hasil jika ada data
                if counts:
                    print(f"\nProcessing {len(counts)} measurement results...")
                    
                    # Urutkan berdasarkan count tertinggi
                    sorted_counts = sorted(counts.items(), key=lambda x: x[1], reverse=True)
                    
                    # Cek 10 hasil teratas
                    top_n = min(10, len(sorted_counts))
                    print(f"Checking top {top_n} results...")
                    
                    for i, (bin_key, count) in enumerate(sorted_counts[:top_n]):
                        if len(bin_key) < quantum_registers:
                            bin_key = bin_key.ljust(quantum_registers, '0')
                        
                        private_key_hex = binary_to_hex(bin_key)
                        print(f"\nChecking result {i+1} (count: {count}):")
                        print(f"Binary key: {bin_key[:20]}...")
                        print(f"Private key: {private_key_hex}")
                        
                        compressed_address = private_key_to_compressed_address(private_key_hex)
                        
                        if compressed_address == target_address:
                            print(f"\n{'!'*60}")
                            print(f"SUCCESS! Found matching private key!")
                            print(f"Private Key: {private_key_hex}")
                            print(f"Bitcoin Address: {compressed_address}")
                            print(f"{'!'*60}")
                            
                            with open('boomqft.txt', 'w') as f:
                                f.write(f"Found private key: {private_key_hex}\n")
                                f.write(f"Corresponding Bitcoin address: {compressed_address}\n")
                            
                            return private_key_hex
                    
                    print(f"\nNo matching key found in top {top_n} results")
                    
                    # Jika ingin cek semua hasil, uncomment bagian ini
                    # for bin_key, count in sorted_counts:
                    #     if len(bin_key) < quantum_registers:
                    #         bin_key = bin_key.ljust(quantum_registers, '0')
                    #     
                    #     private_key_hex = binary_to_hex(bin_key)
                    #     compressed_address = private_key_to_compressed_address(private_key_hex)
                    #     
                    #     if compressed_address == target_address:
                    #         print(f"\n{'!'*60}")
                    #         print(f"SUCCESS! Found matching private key!")
                    #         return private_key_hex
                        
                else:
                    print("No counts data available to check")
                
                print(f"Attempt {attempt} completed, no match found")
                
        except Exception as e:
            print(f"✗ Error in quantum execution: {e}")
            import traceback
            traceback.print_exc()
            
            # Jika error karena backend tidak tersedia, coba backend lain
            if "not found" in str(e) or "unavailable" in str(e):
                print("Backend mungkin tidak tersedia, mencoba backend lain...")
                # Cari backend alternatif
                try:
                    backends = service.backends()
                    hardware_backends = [b for b in backends if not b.configuration().simulator and b.name != backend.name]
                    if hardware_backends:
                        backend = hardware_backends[0]
                        print(f"Switching to backend: {backend.name}")
                    else:
                        print("No alternative hardware backends available")
                        return None
                except:
                    print("Could not find alternative backend")
        
        # Delay sebelum attempt berikutnya
        if not backend.configuration().simulator:
            print("Waiting 60 seconds before next attempt...")
            time.sleep(60)
        else:
            print("Waiting 5 seconds before next attempt...")
            time.sleep(5)

def main():
    target_address = '16RGFo6hjq9ym6Pj7N5H7L1NR1rVPJyw2v'
    public_key_x_hex = "02145d2611c823a396ef6712ce0f712f09b9b4f3135e3e0aa3230fb9b6d08d1e16"
    public_key_x = int(public_key_x_hex[2:], 16)
    
    # Elliptic curve parameters
    g_x = 0x79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798
    g_y = 0x483ADA7726A3C4655DA4FBFC0E1108A8FD17B448A68554199B0C75643B8F8E4F
    p = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F
    min_range = 0x4000000000000000000000000000000000
    max_range = 0x7fffffffffffffffffffffffffffffffff

    print("="*60)
    print("Starting quantum brute-force search...")
    print("="*60)
    print(f"Target Bitcoin Address: {target_address}")
    print(f"Public Key X: {public_key_x_hex}")
    print(f"Search Range: {hex(min_range)} to {hex(max_range)}")
    print(f"Range size: {max_range - min_range:,} keys")
    print("="*60)
    
    private_key = quantum_brute_force(public_key_x, g_x, g_y, p, min_range, max_range)

    if private_key is not None:
        private_key_hex = f"{private_key:064x}"
        print(f"\n{'*'*60}")
        print("SEARCH COMPLETED SUCCESSFULLY!")
        print(f"Private key: {private_key_hex}")
        found_address = private_key_to_compressed_address(private_key_hex)
        print(f"Bitcoin address: {found_address}")
        print(f"{'*'*60}")
        
        # Simpan hasil ke file
        with open('found_key.txt', 'w') as f:
            f.write(f"Private Key: {private_key_hex}\n")
            f.write(f"Bitcoin Address: {found_address}\n")
            f.write(f"Public Key X: {public_key_x_hex}\n")
            f.write(f"Found at: {time.ctime()}\n")
            
    else:
        print(f"\n{'*'*60}")
        print("Search completed.")
        print("No private key found in the specified range.")
        print(f"{'*'*60}")

if __name__ == "__main__":
    main()
